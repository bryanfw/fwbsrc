function [deformedIm, XX, YY, ZZ] = vu3DAdaptiveBasesRegistrationWithRadius(fixedIm,movingIm,XX,YY,ZZ,varargin)
% vu3DAdaptiveBasesRegistrationWithRadius(...) performs a deformable registration
% between input images
%
%   SYNTAX:
%       [deformedIm, XX, YY, ZZ] = vu3DAdaptiveBasesRegistrationWithRadius(fixedIm, movingIm, XX, YY, ZZ)
%       [deformedImIm, XX, YY, ZZ] = vu3DAdaptiveBasesRegistrationWithRadius(fixedIm, movingIm, XX, YY, ZZ, options)
%
%   PARAMETERS:
...
p.FunctionName='vu3DAdaptiveBasesRegistrationWithRadius';
p.parse(varargin{:});
...
elseif (strcmpi(p.Results.optimizer,'conjugate_gradient'))
    ...
    error('Matlab:vu3DAdaptiveBasesRegistrationWithRadius:Optimizer','The optimizer chosen was not reconized');
end
...
for level = 1:size(p.Results.gridResolutions,2)
    ...
    if (isempty(rbfZLocations))
        rbfZLocations = sizeFixed(3)/2;
    end
    ...
    while(~isempty(gradientSquareNormGrid))
		...
		rbfZLocations = rbfZLocations(rbfZZIndicesKeep(:));
        
        if (size(rbfZLocations,2)==1) 
            rbfZLocations = rbfZLocations';
        end
		...
		PowellCoeff = MEX3DRadialBasesOptimizationWithRadius(rbfXLocations,rbfYLocations, rbfZLocations, ...
            rbfWeights,p.Results.rbfRadiusPercent, movingIm, fixedIm, deformedIm, ...
             ...
             vuRowMajorColumnMajorSwitch(p.Results.mask), gridSpacing, minmax);
