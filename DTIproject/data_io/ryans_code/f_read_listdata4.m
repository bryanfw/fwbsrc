...
function [varargout] = f_read_listdata4(dataname_s,varargin)
%[f_read_listdata4] parses LIST file and read DATA data.
%
% This file initially comes from [listparser.m].
%
% USAGE
%   [varargout] = f_read_listdata4(dataname_s)
%   [std_data] = f_read_listdata4(dataname_s,{'std'})
%   [phc_data,std_data] = f_read_listdata4(dataname_s,{'phc','std'},loadopts)
%
% INPUT
%   dataname_s: 
...
elseif nargin==3
    ...
    error('Number of input must be 1 or 2 or 3')
end
...
if flag_read_list == 1 % read LIST file
    ...
    while ~feof(fid)
        ...
        if tmp_count==1
            ...
            if strcmpi(typ,'noi')
                tmp_attr_m = zeros(nNOI,length(attr)+1); % +1 for index in LIST file
                fprintf('  attr matrix is generated for %s\n',upper(typ))
            elseif strcmpi(typ,'phc')
                tmp_attr_m = zeros(nPHC,length(attr)+1);
                fprintf('  attr matrix is generated for %s\n',upper(typ))
            elseif strcmpi(typ,'std')
                tmp_attr_m = zeros(nSTD,length(attr)+1);
                fprintf('  attr matrix is generated for %s\n',upper(typ))
            elseif strcmpi(typ,'nav')
                tmp_attr_m = zeros(nNAV,length(attr)+1);
                fprintf('  attr matrix is generated for %s\n',upper(typ))
            elseif strcmpi(typ,'frc')
                tmp_attr_m = zeros(nFRC,length(attr)+1);
                fprintf('  attr matrix is generated for %s\n',upper(typ))
            else
        ...
        % Final attr matrix.
        if strcmpi(typ,'noi') && tmp_count==nNOI
            noi_attr_m = tmp_attr_m;
            noi_count = tmp_count; tmp_count=0;
            clear  tmp_attr_m
        elseif strcmpi(typ,'phc') && tmp_count==nPHC
            phc_attr_m = tmp_attr_m;
            phc_count = tmp_count; tmp_count=0;
            clear  tmp_attr_m
        elseif strcmpi(typ,'std') && tmp_count==nSTD
            std_attr_m = tmp_attr_m;
            std_count = tmp_count; tmp_count=0;
            clear  tmp_attr_m
        elseif strcmpi(typ,'nav') && tmp_count==nNAV
            nav_attr_m = tmp_attr_m;
            nav_count = tmp_count; tmp_count=0;
            clear  tmp_attr_m
        elseif strcmpi(typ,'frc') && tmp_count==nFRC
            frc_attr_m = tmp_attr_m;
            frc_count = tmp_count; tmp_count=0;
            clear  tmp_attr_m
        end
    end
    t=toc;
