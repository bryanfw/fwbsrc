...
jIcon=javax.swing.ImageIcon( sprintf('%s/MURDOCH/philips_shield_logo_16x16.gif',ctfroot) );
jframe.setFigureIcon(jIcon);

name = inputname(1);
fig_name = 'MURDOCH Explore';
fig_name = strcat(fig_name, ' : ', name);
set(h,  'Name', 'MURDOCH Explore PARS');
...
txtvalue = uicontrol('String', '', 'Units', 'normalized', 'Style', 'Edit',...
                            'Position', [.491 .85 3*.145 .034], 'BackgroundColor',  [.925 .914 .847]);
colvalue = uicontrol('String', 'Value', 'Units', 'normalized', 'Style', 'Text',...
                            'Position', [.375 .85 .1 .034], 'BackgroundColor',  [.925 .914 .847]);  
...
set(tree.FigureComponent,'UIContextMenu', cmenu);
t = tree.Tree;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function cNode = nodeSelected_cb4(tree,ev)
        ...
        if ischar(val),
            set(txtvalue, 'string', val);
        elseif isnumeric(val),
            set(txtvalue, 'string', num2str(val));
        else
            set(txtvalue, 'string', '');
        end
