...
% define, then clean attr names (will be used as variablenames and fieldnames)
...
order = {'kx','ky','kz','loca','dyn','card','echo','mix','aver','chan','extr1','extr2'};

% determine size of 'data' variable and initialize it
% this could be re-written to be a little clearer and avoid wierd eval statments
tmpstr = '';
...
% for each attr adjust indicies so that they are >= 1 (except for kx)
for k=1:length(order),
    if strcmp(order{k},'kx')==0,% not kx
...
% lets start on this data file now that we have "data" variable
...
N = length(idxSTD);
for n=1:N, % for each STD line in the big file
    if( fseek(fid, str2double( info(idxSTD(n)).offset ) ,'bof') == 0) % if successfully pointed to a STD line
        
        tmpdata = fread(fid, str2double( info(idxSTD(n)).size )/4 ,'float32'); %
        tmpdata = tmpdata(1:2:end) + 1i*tmpdata(2:2:end); % real & imag data
        tmpdata = tmpdata * str2double( info(idxSTD(n)).sign );
