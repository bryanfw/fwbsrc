%
% Advanced IEPI Reconstruction Script - Addition Of Golden Section Search
%
%
...
%--------------------------------------------------------------------------------------
% INPUT Management - pick the image from the series to work on
%--------------------------------------------------------------------------------------
...
%----------------------------------------------------------------------------------------------
% IMPROVE LINEAR PHASE CORRECTION
%----------------------------------------------------------------------------------------------
FPC_RANGE =  abs(max(Fp-Fn));
FPC_TOL   =  0.02;

low_x     = -0.382*FPC_RANGE;
hig_x     =  0.618*FPC_RANGE;

for S = 1:Ns
    pim     = squeeze(fpc_pimage(S,:,:));
    nim     = squeeze(fpc_nimage(S,:,:));
    FPC(S)  = golden_section('FPC_function',low_x,0.0,hig_x,FPC_TOL,Z_min(S),pn_filter,pim,nim,x_matrix,y_matrix);
end


%----------------------------------------------------------------------------------------------
% IMPROVE ZERO PHASE CORRECTION
%----------------------------------------------------------------------------------------------
ZPC_TOL   =  0.02;
low_x     = 0.0;
hig_x     = 2*pi;
...
for S = 1:Ns
    pim     = squeeze(fpc_pimage(S,:,:));
    nim     = squeeze(fpc_nimage(S,:,:));  
    ZPC(S)  = golden_section('ZPC_function',low_x,Z_min(S),hig_x,ZPC_TOL,FPC(S),pn_filter,pim,nim,x_matrix,y_matrix);
end
...
for S = 1:Ns
    pim              = squeeze(fpc_pimage(S,:,:));
    nim              = squeeze(fpc_nimage(S,:,:));
    zpc_image(S,:,:) = PC_image(ZPC(S),FPC(S),pim,nim,x_matrix,y_matrix);
end
...
for S = 1:Ns
    pim         = squeeze(fpc_pimage(S,:,:));
    nim         = squeeze(fpc_nimage(S,:,:));
    rec1(S,:,:) = PC_image(Z_min(S),0.0,pim,nim,x_matrix,y_matrix);
    rec2(S,:,:) = zpc_image(S,:,:);
    abs_scale1  = GRAY_RANGE/(max(max(max(abs(rec1)))));
    abs_scale2  = GRAY_RANGE/(max(max(max(abs(rec2)))));

plot_index = (S-1)*Ns + 1;

    subplot(Ns,2,plot_index);
    pic = GRAY_CENTER + abs_scale1*abs(squeeze(rec1(S,:,:)));
    image(pic);
    axis('image')
    colormap(gray(GRAY_VALUES));
    title(['recon 1 - shot ',int2str(S)]);

plot_index = (S-1)*Ns + 2;   

    subplot(Ns,2,plot_index);
    pic = GRAY_CENTER + abs_scale2*abs(squeeze(rec2(S,:,:)));
    image(pic);
    axis('image')
    colormap(gray(GRAY_VALUES));
    title(['recon 2 - shot ',int2str(S)]);    
