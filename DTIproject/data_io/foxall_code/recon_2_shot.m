%
% Basic IEPI Reconstruction Script For 2 Shot Case
%
%



function recon_results = recon_2_shot(data_iepi,echo_sign,data_des)
...
for Z = 1:Nz
    ...
    Qp(S,Z) = sum_Q(zpc_im.*pn_filter);
end
    

    [min_val,min_index] = min(Qp(S,:));
...
recon_results.combined_image = cpc_image;
recon_results.Fp             = Fp;
recon_results.Fn             = Fn;
recon_results.Z              = Z_min;
recon_results.Qp             = Qp;
recon_results.Zp             = Zp;
recon_results.C              = C_min;
recon_results.Qc             = Qc;
recon_results.Zc             = Zc;
