...
function [recon_6d,gfactor_6d] = f_sense_recon_v3(sense_7d, smap_4d, psi_m, ...
    mask_3d, R, SMAPparams, GENparams, DATAFLAGparams, REFparams, DWIparams, dw_ori, nav_size_v)
%[f_sense_recon_v3] performs conventional sense reconstruction.
%
% USAGE
%   [recon_6d,gfactor_6d] = f_sense_recon_v3( ...
...
% 2012.07.24.
%   Just load I_nav_sense_7d from disk. Then the input is just dummy. This
%   version [f_sense_recon_v3.m] is generated for 3D acq.
%
% Ha-Kyu
...
if strcmpi(DWIparams.scan_mode,'3D')
    ...
    %nnchunk = DWIparams.chunks;
    nny = nav_size_v(1);
    nnx = nav_size_v(2);
    nnz = nav_size_v(3);
    nnchunk = nav_size_v(4);
    nnc = nav_size_v(5);
    nnavg = nav_size_v(6);
    nndw = nav_size_v(7);
    nns = nav_size_v(8);
...
if DATAFLAGparams.matchObjPosSMAP_IMG==1
    ...
    for ind0 = 1:nnchunk
        ...
        eval(sprintf('load  I_nav_sense_chunk%.2d_7d.mat',ind0))
        fprintf('      I_nav_sense_chunk%.2d_7d is loaded\n',ind0)
        sense_7d = I_nav_sense_7d;
                ...
                for ind3 = 1:nndw
                    for ind4 = 1:nns
                        if strcmpi(DWIparams.fat_shift_dir,'P')
                            if strcmpi(DWIparams.scan_mode,'3D')
                                ind00 = 1;
                                sense_7d(:,:,:,ind00,ind1,ind2,ind3,ind4) = ...
                                    circshift(squeeze(sense_7d(:,:,:,ind00,ind1,ind2,ind3,ind4)), ...
                                    [-round(delta),0,0]);
                            else
        ...
        % Save temporary shifted nav chunk data
        if ~exist(sprintf('I_nav_sense_chunk%.2d_temp_7d.mat',ind0),'file')
            eval(sprintf('save  I_nav_sense_chunk%.2d_temp_7d.mat  sense_7d',ind0))
            fprintf('      I_nav_sense_chunk%.2d_temp_7d.mat is saved\n',ind0)
        else
            fprintf('      I_nav_sense_chunk%.2d_temp_7d.mat exists\n',ind0)
        end
    ...
    for ind0 = 1:nnchunk
        ...
        if ~exist(sprintf('I_nav_sense_chunk%.2d_temp_7d.mat',ind0),'file')
            eval(sprintf('load  I_nav_sense_chunk%.2d_7d.mat',ind0))
            fprintf('      I_nav_sense_chunk%.2d_7d.mat is loaded\n',ind0)
            sense_7d = I_nav_sense_7d;
            clear  I_nav_sense_7d
            
            % Save temporary unshifted nav chunk data            
            eval(sprintf('save  I_nav_sense_chunk%.2d_temp_7d.mat  sense_7d',ind0))
            fprintf('      I_nav_sense_chunk%.2d_temp_7d.mat is saved\n',ind0)
        else
            fprintf('      I_nav_sense_chunk%.2d_temp_7d.mat exists\n',ind0)
...
for ind_chunk = 1:nnchunk 
    % Load temporary nav chunk data (shifted or unshifted).
    % This will load sense_7d.
    eval(sprintf('load  I_nav_sense_chunk%.2d_temp_7d.mat',ind_chunk))
    fprintf('      I_nav_sense_chunk%.2d_temp_7d.mat is loaded\n',ind_chunk)
    ...
    for ind_dw = 1:nndw
        for ind_avg = 1:nnavg
            for ind_slice = 1:nnz
                        ...
                        for indy = 1:nny
                            ...
                            if any(I_mask_nav_m(rho_v,indx) == 1)
                                ...
                                if strcmpi(DWIparams.scan_mode,'3D')
                                    ...
                                    ind_chunk1 = 1;
                                    m_v = squeeze(sense_7d(indy,indx,ind_slice,ind_chunk1,:,ind_avg,ind_dw,ind_shot));
                                else
                ...
                % Save temporary recon data.
                if ind_avg==1 && ind_dw==1 && ind_chunk==1 && ind_slice==1
                    % this will be for the last shot.
                    eval(sprintf('save  temp_nav_recon  recon_m  I_mask_nav_m'))
                end
    ...
    % Delete temporary nav chunk data (shifted or unshifted).
    if exist(sprintf('I_nav_sense_chunk%.2d_temp_7d.mat',ind_chunk),'file')
        eval(sprintf('delete  I_nav_sense_chunk%.2d_temp_7d.mat',ind_chunk))
        fprintf('      I_nav_sense_chunk%.2d_temp_7d.mat is deleted\n',ind_chunk)
    end
    clear  sense_7d
    pack
