...
% 2008.03.11    store PAR characte text pieces in INFO - welcheb
% 2008.10.13    Modify reading diffusion encoding gradient directions - jeongh1
% 2010.08.09.
%   This is generated from [loadParRec_jeongh1.m].
% 2012.07.06.
...
%% Function definition
function [data,info] = f_read_parrec(filename,varargin)
...
info.partext_char_header = partext_char_header;
info.partext_char_footer = partext_char_footer;
...
toks = regexp(partext_char_header,'^\.\s+([\w\. ]+\w).+:\s+(.+)$','tokens');
for k=1:length(toks), 
    if ~isempty(toks{k}),
        info_fieldname = char(toks{k}{1}{1});
        info_fieldname = strrep(info_fieldname,' ','_');
        info_fieldname = strrep(info_fieldname,'.','');
        info.(info_fieldname) = char(toks{k}{1}{2});
    end
end

%% Detect image information definitions
ncols = 0;
toks = regexp(partext_char_header,'^#\s+([\w\. ]+\w).+\((\d)?\*?(integer|float|string)\)$','tokens');
imgdef_fieldnames = {};
...
for k=1:length(toks),
    if ~isempty(toks{k}),
        ...
        imgdef_fieldname = strrep(imgdef_fieldname,' ','_');
...
info.imgdef.table = zeros(info.n_parrec_imgs,ncols);
for k=1:info.n_parrec_imgs,
    info.imgdef.table(k,:) = str2num(char(partext_numeric{k}));
end
...
for k=1:count_imgdef_fieldnames,
    ...
    vals = info.imgdef.table(:,start_col:stop_col);
    info.imgdef.(imgdef_fieldnames{k}).vals = vals;    
    
    % KEEP ORIGINAL
    %info.imgdef.(imgdef_fieldnames{k}).uniq = unique(vals,'rows');
    
    % VUIIS : jeongh1 : read unique dw gradient orientations, sorted case.
    if k==40    % diffusion gradient orientations
        nSlice = str2double(info.Max_number_of_slices);
        nDir = str2double(info.Max_number_of_gradient_orients);
        uniq_dir_first_sl_v = 1:nSlice:nSlice*nDir;
        info.imgdef.(imgdef_fieldnames{k}).uniq = ...
            info.imgdef.(imgdef_fieldnames{k}).vals(uniq_dir_first_sl_v,:);
    else
        info.imgdef.(imgdef_fieldnames{k}).uniq = unique(vals,'rows');
    end
    % VUIIS : jeongh1 : END
    
    % if size is one, store as row vector for easier display at prompt
...
if isfield(info,'Image_pixel_size'),
    info.pixel_bits = str2num(info.Image_pixel_size);
else
    info.pixel_bits = info.imgdef.table(1,pixel_bits_col);
end
...
if isfield(info,'Recon_resolution'),
    Recon_resolution = str2num(info.Recon_resolution);
    ...
    info.img_pixels = prod(info.imgdef.recon_resolution.uniq);
    info.recon_x = info.imgdef.recon_resolution.uniq(1);
    info.recon_y = info.imgdef.recon_resolution.uniq(2);
    info.dims.x = [1:info.imgdef.recon_resolution.uniq(1)]; 
    info.dims.y = [1:info.imgdef.recon_resolution.uniq(2)];
end

%% Find the unique set of values for each dimension name
for k=1:length(dimnames),
    info.dims.(dimnames{k}) = unique(info.imgdef.table(:,dimcols(k))).';
end
...
info.imgdef.table_row_index_array = zeros(datasize(3:end));
...
for n=1:info.n_parrec_imgs,
    ...
    rec_index = info.imgdef.table(n,rec_index_col);
    
    for k=1:length(dimnames),
        
        dimval = info.imgdef.table(n,dimcols(k));
    ...
    if load_flag,
        ...
        for d=1:size(dim_assign_indices_full_array,1),
            ...
            eval( sprintf('info.imgdef.table_row_index_array(%s)=%d;', dim_assign_str, n) );
...
for k=1:info.n_data_imgs,
    ...
    table_row_index = info.imgdef.table_row_index_array(k);
   
    if(table_row_index>0),
        
        % rescale intercept
        ri = info.imgdef.table(table_row_index,ri_col);

        % rescale slope
        rs = info.imgdef.table(table_row_index,rs_col);

        % scale slope
        ss = info.imgdef.table(table_row_index,ss_col);
...
if info.loadopts.reducesingletons==true,
    for k=1:ndims,
        ...
        if length(info.dims.(dimnames{k}))>1,
            ...
            for n=1:length(info.dims.(dimnames{k})),
                ...
                eval(sprintf('tmp = info.imgdef.table_row_index_array(%s);', dimstr_n) );
    ...
    for k=1:length(last_nonempty_idx),
        if last_nonempty_idx(k)~=0,
            ...
            eval(sprintf('info.imgdef.table_row_index_array = info.imgdef.table_row_index_array(%s);', dimstr_n) );
