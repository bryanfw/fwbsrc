...
function [refparams,dwiparams] = f_read_examcard_params_3T(...
    xmlfile, REFrawfilename, DWIrawfilename, ...
    REFexamcardindex, DWIexamcardindex, coilType)
%[f_read_examcard_params] reads ExamCard parameters for 3T
%
% USAGE:
...
%   DWIexamcardindex:   Index of the DWI scan in the ExamCard
...
% 2010.12.07.
%   Some error message 'MSGID' are fixed.
% 2010.12.09.
...
% 2011.10.19.
%   Add if exist(... to skip reading the parameters for non-diffusion
%   weighting case.
% 2011.10.31.
                ...
                switch param_name_s
                    case 'Stacks'
                        ...
                        if isfield(struct_temp,'subparameter')
                            ...
                            error('f_read_examcard_params_3T:main','''Stacks'' parameter must have ''subparameter''')
                        end
                        clear  struct_temp
                    ...
                    case 'FOV          FH (mm)' % coronal RL-F or RL-H | sagital AP-F or AP-H
                        ...
                        if isfield(struct_temp,'subparameter')
                            ...
                            error('f_read_examcard_params_3T:main','''FOV'' parameter must have ''subparameter''')
                        end
                    ...
                    case 'Voxel size   FH (mm)' % coronal RL-F or RL-H | sagittal AP-F or AP-H
                        ...
                        if isfield(struct_temp,'subparameter')
                            ...
                            error('f_read_examcard_params_3T:main','''Voxel size'' parameter must have ''subparameter''')
                        end
...
if ~isempty(DWIrawfilename)
                ...
                switch param_name_s
                    ...
                    case 'Fast Imaging mode'
                        ...
                        if isfield(struct_temp,'subparameter')
                            ...
                            error('f_read_examcard_params_3T:main','''Fast Imaging mode'' parameter must have ''subparameter''')
                        end
                        clear  struct_temp
                            ...
                            for ind_subparam = 1:nsubparam
                                ...
                                switch subparam_name_s
                                    ...
                                    case 'nr of directions'
                                        number_of_directions = struct_temp.subparameter(ind_subparam).ATTRIBUTE.value;
                                    case 'gradient overplus'
                                        gradient_overplus = struct_temp.subparameter(ind_subparam).ATTRIBUTE.value; % yes, no
                                end
                            end
                        else
                            error('f_read_examcard_params_3T:main','''Diffusion mode'' parameter must have ''subparameter''')
                        end
                        clear  struct_temp
                end % switch param_name_s
            end % for ind_param
                ...
                switch param_name_s
                    case 'SENSE'
                        ...
                        if isfield(struct_temp,'subparameter')
                            ...
                            error('f_read_examcard_params_3T:main','''SENSE'' parameter must have ''subparameter''')
                        end
                        clear  struct_temp
                    ...
                    case 'Slice thickness (mm)'
                        ...
                        elseif isfield(acq_vox,'AP') && isfield(acq_vox,'FH') % sagittal FH-A or FH-P
                            ...
                            error('f_read_examcard_params_3T:main','''Slice thickness (mm)'' parameter has unknown direction of acq_vox')
                        end
        ...
        elseif strcmpi(tab_name_s,info_name_s)
            ...
            for ind_param = 1:nparam
                ...
                switch param_name_s
                    ...
                    case 'Act. WFS (pix) / BW (Hz)' % other case, e.g., FFE with EPI readout
                        struct_temp = examcard.protocol(examcardindex_DWI). ...
                            group(ind_group).parameter(ind_param);
    ...
    dwiparams.fov = fov;
    if exist('rfov','var'),dwiparams.rfov = rfov;end
