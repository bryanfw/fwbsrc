...
datapath = '~/mounts/fs2/masi/bryanfw/spleen_data/';
origdatapath = [datapath 'raw/'];
truepath = [datapath 'label/'];

origdatadir = dir([origdatapath '*.nii.gz']);
truedir = dir([truepath '*.nii.gz']);

truedir = struct2cell(truedir); truedir = truedir(1,:);

for ii = 1:length(origdatadir);
    obs.intensity{ii} = [origdatapath origdatadir(ii).name];
    id = regexp(origdatadir(ii).name,'_');
    id = origdatadir(ii).name(1:(id(1)-1));
    
    disp(str2double(id(2:5)))
    obs.subid(ii) = str2double(id(2:5));
    obs.scanid(ii) = str2double(id(7:end));
        
    % find corresponding truth-rated labels 
    ind = find(not(cellfun('isempty',strfind(truedir,id))));
    if ~isempty(ind); 
        obs.truth{ii} = [truepath truedir{ind}];
    end
end
